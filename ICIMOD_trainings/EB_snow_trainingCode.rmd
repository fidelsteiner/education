...
TCNR4hourly <- TSaggregate(datRaw$CNR4TC_Avg,timestr,60,5,'mean')         # TCNR4, mean CNR4 temperature, sensor for radiation correction [degC]
...
H = rho * Cp * c_bt * expData$WSPD * (expData$TAIR - expData$TSURF)      # sensible heat flux
LE = rho * LL * c_bt * expData$WSPD * (q - qs)                           # latent heat flux


#### Net Radiation

Rad_net <- expData$KINC - expData$KUPW + expData$LINC - expData$LUPW

Energy_n <- Rad_net + H + LE
...
for(i in 2:length(actualSD)){
  ...
  countTot <- countTot + 1                              # Count number of hours that sufficient snow is present
    if(Energy_n[i]>0.0){                                   # If net energy is positive (adds energy to the surface)
      if(TS[i]>=0.0){                                   # If surface temperature is at melting point (positive net energy is used to melt snow)
        ...
        countC[i] <- countC[i-1] + 1                    # Count number of hours that cold content changes [W m-2]
      ...
      if(actStor[i]>0.0){                               # If liquid water is stored within the snowpack
        ...
        countR <- countR + 1.0                          # Count number of hours that refreezing occurs [h]
    ...
    if(TS[i]<0.0){                                      # If surface temperature smaller than 0.0, then ...
      ...
      if(subl[i]<0.0){                                  # If latent heat is negative, then ...
        ...
        countD <- countD + 1}                           # Count number of hours that deposition [h] 
    ...
    if(evap[i]<0.0){                                    # If latent heat is negative [W m-2]
      ...
      countE <- countE + 1                              # Count number of evaporation occurs
    }else{                                              # If latent heat is positive [W m-2]
      cumD[i] = cumD[i-1] + evap[i]                     # Cumulative condensation []
      countD <- countD + 1}                             # Count number of hours that condensation occurs [h]
